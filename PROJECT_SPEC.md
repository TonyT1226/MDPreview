...
- 任务复选框：SF Symbol 附件，带 `.mdTaskOrdinal`（块内第几个复选框）/ `.mdTaskChecked` 自定义属性；点击时由 `ReaderPieces` 按当前块查出源行号。
...
| G10 | 代码着色为启发式逐行正则，跨行字符串 / 复杂语法会错色；语言有限 | 观感瑕疵 | 暂不排期 |
...
| G17 | 核心库静态链接进 App 和两个扩展，DMG 从约 3.6MB 变成约 7.5MB | 体积 | 暂不排期（可改成动态框架） |
| G18 | 没有公证：DMG 仍是 ad-hoc 签名 | 首次打开要右键 › 打开 | 需要 Developer ID |

**v1.5 已解决**：~~G12 静默刷新标成「已编辑」、文件删后停止监听~~、~~G15 每次改动整篇重排~~。
...
### v1.4 — 编辑体验（开发中，分支 `v1.4`）

| 项 | 落地内容 |
| :-- | :-- |
| **1.4.1 快捷格式** ✅ | 「格式」菜单：`⌘B` 加粗、`⌘I` 斜体、`⇧⌘K` 行内代码、`⌘K` 链接。有选区时包裹、再按一次取消；无选区时插入一对标记。`***x***` 能正确区分加粗与斜体。命令经响应链发给当前编辑器，阅读区、查找栏不受影响。 |
| **1.4.2 列表续写** ✅ | 回车续 `-` `*` `+` / `1.` `1)`（序号 +1）/ 任务项（新项未勾选）；光标在正文中间时后半段带到新项；空项上回车清掉标记。分割线、围栏代码块里不介入。 |
| **1.4.3 Tab 缩进** ✅ | 光标在列表项上或选区跨多行时，`Tab` / `⇧Tab` 整行加减 4 个空格（对 `- ` 与 `1. ` 都是合法嵌套）；其他情况仍插入制表符。 |
| **1.4.4 源码着色修正** ✅ (原 G11) | 原 G11 的「围栏只染 ``` 行」描述有误：实测围栏整块本来就会着色。真正的问题是 `**粗**` 被斜体规则匹配成斜体色，已修；行内代码里的标记不再着色；token 按位置排序，排版时只取落在该段的 token。160KB 文档整篇重扫约 19ms，所以没有做增量重扫，只把重扫延迟从 120ms 降到 60ms。 |
| **1.4.5 粘贴 / 拖入图片** ✅ | 编辑器改用 `MarkdownTextView`（`NSTextView` 子类，仍由 `scrollableTextView()` 创建，保持 TextKit 2）接管粘贴与拖放。图片存到 `<文档目录>/assets/`，文件名「文档名-时间」，同名顺延不覆盖，插入 `![](assets/…)`。插入前弹说明窗，带「不再显示」（只在选「插入」时记住；偏好设置里可重新打开）。未保存的文档提示先保存。粘贴板同时有文字时（Excel、网页）仍按文字粘贴。 |

实现上，快捷格式 / 续写 / 缩进的规则都在 `MarkdownEditing`（文本 + 选区 → 一次替换），编辑器用 `shouldChangeText` 应用，保证一次撤销；输入法组字时一律不介入。

### v1.5 — 性能与热重载（开发中，分支 `v1.5`）

| 项 | 落地内容 |
| :-- | :-- |
| **1.5.1 阅读区增量更新** ✅ (G15) | `ReaderRenderer.renderPiece` 按顶层块渲染；`ReaderPieces` 记住每块的片段，新块列表到来时从两端比对（`rendersSame`：不比 id 和列表项行号），只重渲染中间改动的块，只替换 text storage 里对应的一段。复选框改记「块内第几个」（`.mdTaskOrdinal`），点击时按当前块查行号，所以上方插一行不会让下面的块作废。远程图片加载完只重渲染含图片的块。行内代码、`==高亮==` 的底色改成共用对象（动态颜色按对象比较，每次新建会让所有块都被当成改过）。 |
| **1.5.2 热重载** ✅ (G12) | 静默刷新和冲突框里的「使用磁盘版本」走 `NSDocument.revert`（与「复原到上次存储的版本」同一条路），不再标成「已编辑」；找不到 NSDocument 或 revert 失败时退回直接改文本。`FileMonitor` 在文件被删后监听所在目录，同名文件再出现时重新挂载并报 `.modified`。 |

性能（`MDP_PERF=1 swift test --filter PerfProbe`，Debug 构建，160KB 文档，视口在文档中部，改一个字）：
整篇重排约 0.8 秒，增量更新约 30 毫秒。解析仍是整篇（约 180 毫秒），在后台线程，不占主线程。

### 暂不排期

- 核心库改动态框架（G17）；代码高亮改词法分析（G10）。
- 行内图片、单段多图、图片点击查看（G8）；数学公式；HTML 块有限渲染。
- 编辑器当前行整行高亮（需要换掉 `scrollableTextView()`，v1.2 试过会冲突）；括号自动配对（易与输入法冲突）。
