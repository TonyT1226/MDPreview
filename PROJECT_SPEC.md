...
| G12 | 热重载静默 reload 会把文档标记为「已编辑」；文件被删后 monitor 停止，直到再次保存才恢复 | 轻微 UX 瑕疵 | **v1.5** |
| G14 | 代码块不横向滚动，长行折行显示；`NSTextBlock` 没有圆角 | 与 v1.2 的 SwiftUI 版观感不同 | 接受 |
| G15 | 每次文本变化都重排整篇属性串；超大文档（数百 KB）在分屏模式下打字时，阅读区更新可能跟不上 | 超大文档手感 | **v1.5** |
...
| 项 | 内容 | 要点 |
| :-- | :-- | :-- |
| **1.5.1 阅读区增量重排** (G15) | 只重新生成改动的块，替换属性串中对应一段 | 块的「内容指纹」不含 id，前后比对找改动段。先加性能测试定目标（如 160KB 文档改一个字 < 50ms），做完用数字验证 |
| **1.5.2 热重载小毛病** (G12) | 静默刷新后不再标成「已编辑」；文件被删后又出现时恢复监听 | 前者要走底层 `NSDocument` 的 revert，属实验项；走不通就保持现状并记录原因 |
