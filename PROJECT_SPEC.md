...
| G11 | 编辑器源码着色：跨段围栏代码块只把 ``` 行本身变灰；超大文件按键时整篇重扫 | 长代码块观感、超大文件手感 | **v1.4** |
...
| 项 | 内容 | 要点 |
| :-- | :-- | :-- |
| **1.4.1 快捷格式** | 新增「格式」菜单：`⌘B` 加粗、`⌘I` 斜体、`⌘K` 链接、`⇧⌘K` 行内代码 | 有选区时包裹，再按一次取消；无选区时插入一对标记、光标放中间。`⌘E` 已用于编辑模式 |
| **1.4.2 列表续写** | 回车续 `- ` / `1.`（序号 +1）/ `- [ ] `；空列表项上回车退出列表 | 输入法组字时不介入（v1.1 修过的坑，要专门测） |
| **1.4.3 Tab 缩进** | 光标在列表项上或选中多行时，`Tab` / `⇧Tab` 整行缩进 / 反缩进 | 其他位置的 Tab 行为不变 |
| **1.4.4 粘贴 / 拖入图片** | 图片存到文档旁的 `assets/`，插入 `![](assets/…)` | 未保存的新文档先提示保存；文件名「文档名-时间」，不覆盖已有文件 |
| **1.4.5 源码着色改进** (G11) | 围栏代码块整块染色；按键时只重扫改动附近的行 | 每行记「是否在围栏内」，从改动处往下扫到状态稳定为止 |

建议顺序：1.4.1 → 1.4.2 → 1.4.3 → 1.4.5 → 1.4.4。
